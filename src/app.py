...
def get_risk_status(pct):
    """Convert percentage to status: SAFE, WARNING, DANGER."""
    if pct >= 75:
        return "DANGER"
    elif pct >= 40:
        return "WARNING"
...
def risk_color(pct):
    """Get color for risk percentage (legacy function for total risk)."""
    status = get_risk_status(pct)
    return get_status_color(status)

def risk_emoji(pct):
    """Get emoji and label for risk percentage (legacy function for total risk)."""
    status = get_risk_status(pct)
    if status == "DANGER":
        return "🔴", "High Risk"
    elif status == "WARNING":
...
def generate_recommendations(row):
    """Generate risk-specific recommendations for 4 construction risks.
    
    Returns a list of 4 recommendation items:
    - Crane risk recommendation
    - Freeze risk recommendation
    - Heat risk recommendation
    ...
    crane_pct = float(row.get("crane_pct", 0) or 0)
    freeze_pct = float(row.get("freeze_pct", 0) or 0)
    heat_pct = float(row.get("heat_pct", 0) or 0)
    flood_pct = float(row.get("flood_pct", 0) or 0)
    
    # CRANE RISK
    crane_status = get_risk_status(crane_pct)
    crane_color = get_status_color(crane_status)
    crane_icon = get_status_icon(crane_status)
    
    if crane_status == "SAFE":
        crane_title = "Crane operations can continue"
        crane_desc = "Wind conditions are within safe limits. Continue standard lifting operations with normal monitoring."
    elif crane_status == "WARNING":
        crane_title = "Limit heavy crane lifts"
        crane_desc = "Wind risk is elevated. Avoid high-altitude or heavy lifting and monitor gusts frequently."
    else:  # DANGER
        crane_title = "Stop crane operations"
        crane_desc = "Unsafe wind conditions detected. Suspend crane activity and secure loads, boom, and lifting equipment."
    
    crane_conf = int(min(98, max(75, 70 + crane_pct * 0.25)))
    recs.append({
        "icon": crane_icon,
        "bar_color": crane_color,
        "title": f"Crane Risk — {crane_status}",
        "desc": crane_desc,
        "conf": crane_conf,
        "risk_name": "Crane",
        "status": crane_status,
        "action_title": crane_title,
    })
    
    # FREEZE RISK
    freeze_status = get_risk_status(freeze_pct)
    freeze_color = get_status_color(freeze_status)
    freeze_icon = get_status_icon(freeze_status)
    
    if freeze_status == "SAFE":
        freeze_title = "Concrete work can continue"
        freeze_desc = "Freezing risk is low. Standard curing and material handling procedures are acceptable."
    elif freeze_status == "WARNING":
        freeze_title = "Protect fresh concrete"
        freeze_desc = "Cold conditions may affect curing. Use covers, monitor concrete temperature, and avoid late-day pouring."
    else:  # DANGER
        freeze_title = "Delay concrete pouring"
        freeze_desc = "High freezing risk detected. Postpone concrete work or use heated curing and insulation systems."
    
    freeze_conf = int(min(98, max(75, 70 + freeze_pct * 0.25)))
    recs.append({
        "icon": freeze_icon,
        "bar_color": freeze_color,
        "title": f"Freeze Risk — {freeze_status}",
        "desc": freeze_desc,
        "conf": freeze_conf,
        "risk_name": "Freeze",
        "status": freeze_status,
        "action_title": freeze_title,
    })
    
    # HEAT RISK
    heat_status = get_risk_status(heat_pct)
    heat_color = get_status_color(heat_status)
    heat_icon = get_status_icon(heat_status)
    
    if heat_status == "SAFE":
        heat_title = "Outdoor work can continue"
        heat_desc = "Heat risk is low. Continue normal operations with standard hydration practices."
    elif heat_status == "WARNING":
        heat_title = "Increase worker heat precautions"
        heat_desc = "Heat stress risk is elevated. Add water breaks, shaded rest areas, and schedule heavy tasks earlier."
    else:  # DANGER
        heat_title = "Restrict heavy outdoor labor"
        heat_desc = "Dangerous heat conditions detected. Reduce exposure, reschedule heavy work, and apply heat safety protocols."
    
    heat_conf = int(min(98, max(75, 70 + heat_pct * 0.25)))
    recs.append({
        "icon": heat_icon,
        "bar_color": heat_color,
        "title": f"Heat Risk — {heat_status}",
        "desc": heat_desc,
        "conf": heat_conf,
        "risk_name": "Heat",
        "status": heat_status,
        "action_title": heat_title,
    })
    
    # FLOOD RISK
    flood_status = get_risk_status(flood_pct)
    flood_color = get_status_color(flood_status)
    flood_icon = get_status_icon(flood_status)
    
    if flood_status == "SAFE":
        flood_title = "Ground work can continue"
        flood_desc = "Flood risk is low. Excavation, logistics, and material movement can continue with normal drainage checks."
    elif flood_status == "WARNING":
        flood_title = "Prepare drainage and protect materials"
        flood_desc = "Rainfall risk is elevated. Avoid deep excavation, inspect drainage, and move materials away from low areas."
    else:  # DANGER
        flood_title = "Stop excavation and low-area work"
        flood_desc = "High flood risk detected. Suspend excavation, protect equipment, and secure drainage paths immediately."
    
    flood_conf = int(min(98, max(75, 70 + flood_pct * 0.25)))
    recs.append({
        "icon": flood_icon,
        "bar_color": flood_color,
        "title": f"Flood Risk — {flood_status}",
        "desc": flood_desc,
        "conf": flood_conf,
        "risk_name": "Flood",
        "status": flood_status,
        "action_title": flood_title,
    })
...
# HERO
# ─────────────────────────────────────────────────────────────────
# City selection buttons (will be positioned over hero via CSS)
...
ah   = (
    ...
    '<div class="sec-sub">Risk-specific construction actions for the selected city and day</div></div>'
    '<div class="ai-wrap">'
    '<div class="ai-head">'
    '<div class="ai-title-grp">'
    '<div class="ai-icon">✦</div>'
    '<div><div class="ai-ttl">Action Items for This Day</div>'
    '<div class="ai-st">Risk-specific actions based on forecast</div></div>'
    '</div>'
